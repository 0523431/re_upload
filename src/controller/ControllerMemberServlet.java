...
@WebServlet(urlPatterns= {"*.mem"}, initParams= {@WebInitParam(name="properties", value="member.properties")})

public class ControllerMemberServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	   
	// Properties : map객체로써, HashMap의 하위 객체
	private Properties pr = new Properties();
	private MemberAction action = new MemberAction();
    ...
    public ControllerMemberServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String command = null;
		try {
			System.out.println(request.getRequestURI()); // /s_project/member/join.do
			System.out.println(request.getContextPath()); // 프로젝트 이름 : /s_project
			...
			Method method = MemberAction.class.getMethod(methodName, paramType);
			// invoke : 메서드 호출 기능
		...
		if(forward !=null) {
			if(forward.isRedirect()) {
				...
				if(forward.getView() == null) {
					forward.setView(command.replace(".mem", ".jsp"));
				}
